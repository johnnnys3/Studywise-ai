  ...
  return (
    <AppShell title="Quizzes & Flashcards">
      ...
      <form onSubmit={createQuiz} className="flex flex-col gap-4 rounded-lg border border-study-line bg-white p-5 md:flex-row md:items-end">
        <label className="text-sm font-medium text-study-navy">
          Questions
          <input type="number" min={1} max={20} value={questionCount} onChange={(event) => setQuestionCount(Number(event.target.value))} className="mt-2 block w-32 rounded border border-study-line px-3 py-2" />
        </label>
        <label className="text-sm font-medium text-study-navy">
          Difficulty
          <select value={difficulty} onChange={(event) => setDifficulty(event.target.value)} className="mt-2 block w-40 rounded border border-study-line px-3 py-2">
            <option value="easy">Easy</option>
            <option value="medium">Medium</option>
            <option value="hard">Hard</option>
          </select>
        </label>
        <button disabled={loading} className="rounded bg-study-navy px-5 py-2.5 text-sm font-semibold text-white hover:bg-black disabled:opacity-60">
      ...
      {quiz ? (
        <section className="mt-8 space-y-5">
          {quiz.questions.map((question, index) => (
            <article key={question.id} className="rounded-lg border border-study-line bg-white p-6">
              ...
              <div className="mt-4 grid gap-2">
                {question.options_json.map((option) => (
                  <label key={option} className="flex items-center gap-3 rounded border border-study-line p-3 text-sm text-slate-700 hover:bg-slate-50">
          ...
          <button onClick={submitQuiz} className="rounded bg-study-navy px-5 py-2.5 text-sm font-semibold text-white hover:bg-black">Submit answers</button>
        </section>
      ) : null}
    </AppShell>
  );
