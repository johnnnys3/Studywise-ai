...
export default function UploadDocumentPage() {
  ...
  return (
    <AppShell title="Upload Document">
      ...
      />
      <form onSubmit={submit} className="rounded-lg border border-study-line bg-white p-8">
        <label className="grid cursor-pointer place-items-center rounded-lg border border-dashed border-slate-300 bg-slate-50 px-6 py-16 text-center hover:bg-white">
          <UploadCloud className="text-study-accent" size={36} />
          <span className="mt-4 block text-lg font-semibold text-study-navy">{file ? file.name : "Choose a PDF or TXT file"}</span>
        ...
        <button disabled={loading} className="mt-6 rounded bg-study-navy px-5 py-2.5 text-sm font-semibold text-white hover:bg-black disabled:opacity-60">
