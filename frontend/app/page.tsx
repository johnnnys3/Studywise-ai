...
export default function HomePage() {
  return (
    <main className="min-h-screen bg-study-surface">
      <header className="border-b border-study-line bg-white">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/auth/register" className="rounded bg-study-navy px-4 py-2 text-sm font-semibold text-white hover:bg-black">Get started</Link>
          </div>
        </div>
      </header>

      <section className="mx-auto grid min-h-[calc(100vh-64px)] max-w-7xl grid-cols-1 gap-12 px-5 py-16 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
        <div>
          <p className="text-sm font-semibold uppercase tracking-wide text-study-accent">Citation-backed study assistant</p>
          <h1 className="mt-4 max-w-4xl text-5xl font-semibold leading-tight text-study-navy">
            Turn your study materials into answers, quizzes, flashcards, and progress insights.
          </h1>
          <p className="reading-copy mt-6 max-w-2xl text-xl text-slate-700">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="/auth/register" className="inline-flex items-center gap-2 rounded bg-study-navy px-5 py-3 text-sm font-semibold text-white hover:bg-black">
              Start studying <ArrowRight size={16} />
            </Link>
            <Link href="/ai-pipeline" className="rounded border border-study-line bg-white px-5 py-3 text-sm font-semibold text-study-navy hover:bg-slate-50">
              View RAG pipeline
            </Link>
          </div>
        </div>

        <div className="rounded-lg border border-study-line bg-white p-6 shadow-academic">
      ...
      <section className="border-t border-study-line bg-white py-14">
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-4 px-5 md:grid-cols-2 lg:grid-cols-4">
